 ...
 *
 * Structural tests for the open-items addon.
...
const ADDON_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "addons", "open-items");

describe("open-items addon", () => {
  it("has a valid addon.json manifest", async () => {
    const raw = await readFile(path.join(ADDON_DIR, "addon.json"), "utf8");
    const manifest = JSON.parse(raw);
    assert.equal(manifest.id, "addon.open-items");
  ...
  it("has an HTML entry file", async () => {
    await access(path.join(ADDON_DIR, "open-items.html"));
  });

  it("has a CSS file", async () => {
    await access(path.join(ADDON_DIR, "open-items.css"));
  });

  it("has a JS file", async () => {
    await access(path.join(ADDON_DIR, "open-items.js"));
  });

  it("HTML references the CSS and JS files", async () => {
    const html = await readFile(path.join(ADDON_DIR, "open-items.html"), "utf8");
    assert.ok(html.includes("open-items.css"), "HTML should reference CSS");
    assert.ok(html.includes("open-items.js"), "HTML should reference JS");
  });
  ...
  it("manifest entry matches the HTML filename", async () => {
    const raw = await readFile(path.join(ADDON_DIR, "addon.json"), "utf8");
    const manifest = JSON.parse(raw);
    assert.equal(manifest.entry, "open-items.html");
  });
