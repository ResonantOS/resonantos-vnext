...
describe("discoverAddons()", () => {
  ...
  it("discovers all real addons from browser-first/addons/", async () => {
    // PR A: addon-infrastructure only — no addons registered yet.
    // Addons are added in subsequent PRs (B–J).
    // This test verifies the discovery engine works with an empty addons directory.
    const addons = await discoverAddons();

    assert.equal(addons.length, 0,
      `Expected 0 addons (engine only PR), got ${addons.length}: ${addons.map(a => a.id).join(", ")}`
    );
  });
});
