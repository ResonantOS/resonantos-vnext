...
fn codex_command_path() -> String {
    ...
    if let Ok(home) = std::env::var("HOME") {
        entries.push(format!("{home}/.hermes/node/bin"));
    }
    if let Ok(current) = std::env::var("PATH") {
        entries.extend(current.split(':').map(ToString::to_string));
    }

    let mut seen = HashSet::new();
    entries
        .into_iter()
        .filter(|entry| !entry.trim().is_empty() && seen.insert(entry.clone()))
        .collect::<Vec<_>>()
        .join(":")
}
...
#[derive(Clone, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ProviderRequestAuditRecord {
...
pub(crate) fn query_provider_request_audit(
    app: &AppHandle,
    limit: Option<usize>,
) -> Result<Vec<ProviderRequestAuditRecord>, String> {
    let portable_state = ensure_portable_user_state(app)?;
    let path = std::path::PathBuf::from(portable_state.logs_root).join("provider-requests.jsonl");
    if !path.exists() {
        return Ok(Vec::new());
    }
    let limit = limit.unwrap_or(80).clamp(1, 500);
    let raw = fs::read_to_string(&path)
        .map_err(|error| format!("Failed to read provider audit log: {error}"))?;
    ...
    for line in raw.lines().rev() {
        ...
        if let Ok(record) = serde_json::from_str::<ProviderRequestAuditRecord>(line) {
            records.push(record);
        }
...
fn local_subnet_openai_compatible_candidates() -> Vec<String> {
    [30004, 30000, 30001]
        .into_iter()
        .flat_map(|port| local_subnet_http_candidates(port, true))
        .collect()
}
...
fn remote_openai_compatible_discovery_candidates(configured_base_url: &str) -> Vec<String> {
    ...
    if let Some(same_host) = base_url_with_port(configured_base_url, 30004) {
        push_unique_url(&mut urls, normalized_openai_compatible_base_url(&same_host));
    }
    if let Some(same_host) = base_url_with_port(configured_base_url, 30001) {
        push_unique_url(&mut urls, normalized_openai_compatible_base_url(&same_host));
    }
    for host in [
        "http://gx10.local:30004",
        "http://asus-gx10.local:30004",
        "http://dgx-spark.local:30004",
        "http://gx10-23bd.local:30004",
        "http://gx10.local:30000",
        "http://asus-gx10.local:30000",
        "http://dgx-spark.local:30000",
        "http://gx10-23bd.local:30000",
        "http://gx10.local:30001",
        "http://asus-gx10.local:30001",
        "http://dgx-spark.local:30001",
        "http://gx10-23bd.local:30001",
    ] {
...
async fn discover_remote_openai_compatible_runtime(
    ...
    Err(format!(
        "No OpenAI-compatible llama-server runtime with models was discovered. Tried configured endpoint, common host aliases, ports 30004/30000/30001, and the local /24 subnet. First failures: {}",
...
mod tests {
    ...
    #[test]
    fn codex_command_path_includes_homebrew_node_for_gui_launches() {
        let path = super::codex_command_path();
        ...
        assert!(
            entries
                .iter()
                .position(|entry| *entry == "/opt/homebrew/bin")
                < entries.iter().position(|entry| *entry == "/usr/bin")
        );
    }
    ...
    fn provider_audit_endpoint_host_excludes_paths_and_secrets() {
        assert_eq!(
            endpoint_host(Some("http://192.168.1.77:30004/v1/chat/completions")),
            Some("192.168.1.77:30004".to_string())
        );
        assert_eq!(
    ...
    fn provider_audit_writer_excludes_prompt_messages_and_secret_values() {
        ...
        let request = ProviderServiceChatRequest {
            ...
            api_base_url: Some("http://192.168.1.77:30004/v1".to_string()),
            runtime_node_id: Some("node-provider-asus-gx10-live".to_string()),
            runtime_node_kind: Some("remote-user-owned".to_string()),
            runtime_node_endpoint: Some("http://192.168.1.77:30004/v1".to_string()),
            auth_tier: Some("supported".to_string()),
            model: "Qwen3.6-35B-A3B-Q4_K_M.gguf".to_string(),
        ...
        assert!(payload.contains("\"endpointHost\":\"192.168.1.77:30004\""));
