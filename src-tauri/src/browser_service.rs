...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BrowserNativeClickRequest {
    pub x: f64,
    pub y: f64,
}
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BrowserNativeScrollRequest {
    pub x: f64,
    pub y: f64,
    pub delta_x: f64,
    pub delta_y: f64,
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BrowserNativeTypeTextRequest {
    pub text: String,
}
...
fn find_chromium_binary() -> Option<PathBuf> {
    ...
    if let Some(path) = find_browser_binary_under(&playwright_cache, 5) {
        return Some(path);
    }
...
fn launch_chromium(
    ...
    let debugging_port = allocate_loopback_port()?;
    ...
    while started.elapsed() < Duration::from_secs(12) {
        if let Some(status) = child
            .try_wait()
            .map_err(|error| format!("Failed to check Chromium startup status: {error}"))?
        {
            return Err(format!(
                "Chromium exited before DevTools became available: {status}"
            ));
        }
        if let Some(ws_url) = query_devtools_websocket(debugging_port)? {
            return Ok((BrowserChild { child }, ws_url));
        }
...
fn allocate_loopback_port() -> Result<u16, String> {
    let listener = TcpListener::bind(("127.0.0.1", 0))
        .map_err(|error| format!("Failed to allocate Chromium debugging port: {error}"))?;
    let port = listener
        .local_addr()
        .map_err(|error| format!("Failed to read Chromium debugging port: {error}"))?
        .port();
    drop(listener);
    Ok(port)
}

fn query_devtools_websocket(port: u16) -> Result<Option<String>, String> {
    let mut stream = match TcpStream::connect(("127.0.0.1", port)) {
        ...
        Err(error) => {
            return Err(format!(
                "Failed to connect to Chromium DevTools endpoint: {error}"
            ))
        }
    };
    stream
        .set_read_timeout(Some(Duration::from_secs(2)))
        .map_err(|error| format!("Failed to set Chromium DevTools read timeout: {error}"))?;
    stream
        .set_write_timeout(Some(Duration::from_secs(2)))
        .map_err(|error| format!("Failed to set Chromium DevTools write timeout: {error}"))?;
    let request = format!(
        "GET /json/version HTTP/1.1\r\nHost: 127.0.0.1:{port}\r\nConnection: close\r\n\r\n"
    );
    stream
        .write_all(request.as_bytes())
        .map_err(|error| format!("Failed to request Chromium DevTools version: {error}"))?;
    ...
    loop {
        match stream.read(&mut buffer) {
            ...
            Err(error) => return Err(format!("Failed to read Chromium DevTools version: {error}")),
        }
    }
    ...
    let payload: Value = match serde_json::from_str(body.trim()) {
        Ok(payload) => payload,
