...
const GX10_MODELS: [Gx10LlamaModelSpec; 2] = [
    Gx10LlamaModelSpec {
        ...
        port: 30004,
        path: "/mnt/data/LM-Studio/models/lmstudio-community/Qwen3.6-35B-A3B-GGUF/Qwen3.6-35B-A3B-Q4_K_M.gguf",
        log_path: "/tmp/qwen36-35b-30004.log",
        launch_args: "-c 400000 -ngl 99 --parallel 2 -fa on --cache-type-k q8_0 --cache-type-v q8_0 --jinja --reasoning off --no-webui --metrics",
    },
    Gx10LlamaModelSpec {
        ...
        launch_args: "-c 262144 -ngl 99 --parallel -1",
    },
];
...
pub(crate) fn switch_gx10_llama_model(
    ...
    let started_at = timestamp();
    let command = format!(
        "pkill -f 'llama-server .*--port {port}' 2>/dev/null || true; nohup {bin} -m {model_path} --host 0.0.0.0 --port {port} {launch_args} > {log_path} 2>&1 & sleep 2; curl -sS --max-time 8 http://127.0.0.1:{port}/health",
        bin = LLAMA_SERVER_PATH,
        model_path = model.path,
        port = model.port,
        launch_args = model.launch_args,
...
mod tests {
    ...
    fn gx10_model_specs_are_bound_to_verified_ports() {
        let qwen35 = gx10_model_spec("Qwen3.6-35B-A3B-Q4_K_M.gguf").expect("qwen35 spec");
        let qwen = gx10_model_spec("Qwen3.6-27B-Q4_K_M.gguf").expect("qwen spec");

        assert_eq!(qwen35.port, 30004);
        assert_eq!(qwen.port, 30001);
        assert!(qwen35.path.contains("Qwen3.6-35B-A3B-GGUF"));
